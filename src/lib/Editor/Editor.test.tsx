import { waitFor, render, RenderResult, renderHook } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import '@testing-library/jest-dom'
import Editor from ".";
import { useFormattingController } from "./hooks/useFormattingController";
import FormattingController, { FormattingControllerActions } from "./controllers/FormattingController";
...
describe("formatting", () => {
  test("turn on bold formatting style should add BOLD modifier to text", async () => {
    let [_, component, actions] = await prepareComponentWithFormattingController();
    ...
    let lines = await component.findAllByTestId("wy-editor-line-element");
    let fragmentModifiers = await component.findAllByTestId("wy-editor-line-fragment");
    expect(lines).toHaveLength(1);
    expect(lines[0]).toHaveTextContent("ab12def");
    expect(fragmentModifiers).toHaveLength(2);
    expect(fragmentModifiers[0]).toHaveTextContent("ab12");
    expect(fragmentModifiers[0]).not.toHaveStyle("font-weight: bold");
    expect(fragmentModifiers[1]).toHaveTextContent("def");
    expect(fragmentModifiers[1]).toHaveStyle("font-weight: bold");
  });


  test("multiple calling bold controller action should add and remove BOLD modifier into text - ", async () => {
    let [_, component, actions] = await prepareComponentWithFormattingController();
    ...
    await waitFor(async () => {
      ...
      actions.bold();
      await userEvent.keyboard("908");
    });

    let lines = await component.findAllByTestId("wy-editor-line-element");
    let fragmentModifiers = await component.findAllByTestId("wy-editor-line-fragment");
    expect(lines).toHaveLength(1);
    expect(lines[0]).toHaveTextContent("ab12def32908");
    expect(fragmentModifiers).toHaveLength(4);
    expect(fragmentModifiers[0]).toHaveTextContent("ab12");
    expect(fragmentModifiers[0]).not.toHaveStyle("font-weight: bold");
    expect(fragmentModifiers[1]).toHaveTextContent("def");
    expect(fragmentModifiers[1]).toHaveStyle("font-weight: bold");
    expect(fragmentModifiers[2]).toHaveTextContent("32");
    expect(fragmentModifiers[2]).not.toHaveStyle("font-weight: bold");
    expect(fragmentModifiers[3]).toHaveTextContent("908");
    expect(fragmentModifiers[3]).toHaveStyle("font-weight: bold");
  });

  test("backspace and call bold action should enable bold modifier", async () => {
    let [editor, component, actions] = await prepareComponentWithFormattingController();
    ...
    let fragmentModifiers = await component.findAllByTestId("wy-editor-line-fragment");
    expect(lines).toHaveLength(1);
    expect(lines[0]).toHaveTextContent("ab12");
    expect(fragmentModifiers).toHaveLength(2);
    expect(fragmentModifiers[0]).toHaveTextContent("ab");
    expect(fragmentModifiers[0]).not.toHaveStyle("font-weight: bold");
    expect(fragmentModifiers[1]).toHaveTextContent("12");
    expect(fragmentModifiers[1]).toHaveStyle("font-weight: bold");
  });
});
...
async function prepareComponentWithFormattingController(): Promise<[HTMLElement, RenderResult, FormattingControllerActions]> {
  const { result } = renderHook(() => useFormattingController());
  const [controller, actions, _] = result.current;

  const component = render(<Editor formattingController={controller} />);
  const editor = await component.findByTestId("wy-editor-element");

  await waitFor(async () => {
    await userEvent.click(editor);
  });

  return [editor, component, actions];
}
